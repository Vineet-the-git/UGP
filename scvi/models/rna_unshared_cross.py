...
# VAE model
class RNA_UNSHARED_CROSS(nn.Module):
    ...
    def inference(
        self, x, cross_inf=True, para=None, batch_index=None, y=None, n_samples=1, transform_batch=None
        ...
        if cross_inf:
            assert para is not None, "para should not be None"
            qz_m = para["qz_m"]
            qz_v = para["qz_v"]
            z = para["z_gene"]
            ql_m = para["ql_m"]
    ...
    def forward(
        self, x, local_l_mean, local_l_var, cross_inf = True, para = None,batch_index=None, y=None
        ...
        """
        # check if cross inf = True then para should not be none
        if cross_inf:
            assert para is not None, "para should not be None"
        # Parameters for z latent distribution
        outputs = self.inference(x, cross_inf, para, batch_index, y)
        qz_m = outputs["qz_m"]
        qz_v = outputs["qz_v"]
        ql_m = outputs["ql_m"]
        ql_v = outputs["ql_v"]
        ...
        return reconst_loss, kl_divergence_l, kl_divergence, z, para_out