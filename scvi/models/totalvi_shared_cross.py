...
from scvi.models.modules import DecoderTOTALVI_SHARED, EncoderTOTALVI
...
# VAE model
class TOTALVI_SHARED_CROSS(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # add an assert condition to check if number of input proteins is equal to number of input rna genes
        ...
        self.n_input_shared = n_input_shared
    ...
    def inference(
        ...
        x_extended = torch.cat((x_idx, x_), dim=1)
        y_extended = torch.cat((y_idx, y_), dim=1)

        if cross_inf:
            assert para is not None, "para cannot be none when cross_inf is True"
            qz_m = para["qz_m"]
            qz_v = para["qz_v"]
            ql_m = para["ql_m"]
            ql_v = para["ql_v"]
            z_gene = para["z_gene"]
            z_protein = para["z_protein"]
            z = torch.cat((z_gene, z_protein), dim=0)
            library_gene = para["l"]
            untran_z = para["untran_z"]
            untran_l = para["untran_l"]
        else:
            # Sampling - Encoder gets concatenated genes + proteins
            qz_m, qz_v, ql_m, ql_v, latent, untran_latent = self.encoder(
                torch.cat((x_extended, y_extended), dim=0), batch_index
            )
            z = latent["z"]
        ...
        z_gene = torch.cat((x_idx, z_gene), dim=1)
        z_protein = torch.cat((y_idx, z_protein), dim=1)
        ...
        px_, py_, log_pro_back_mean = self.decoder(z_gene, z_protein, library_gene, batch_index, label)
        px_["r"] = px_r
        py_["r"] = py_r
    ...
    def forward(
        ...
        batch_idx_x: torch.Tensor,
        batch_idx_y: torch.Tensor,
        cross_inf: bool = True,
        para: Dict = None,
        batch_index: Optional[torch.Tensor] = None,
        label: Optional[torch.Tensor] = None,
        ...
        outputs = self.inference(x, y, batch_idx_x, batch_idx_y, cross_inf, para, batch_index, label)
        qz_m = outputs["qz_m"]
        qz_v = outputs["qz_v"]
        ql_m = outputs["ql_m"]
        ql_v = outputs["ql_v"]
        ...
        para_out = {
            ...
            "z_gene": z_gene,
            "z_protein": z_protein,
            "l": outputs["library_gene"],
            "untran_z": outputs["untran_z"],
            "untran_l": outputs["untran_l"],
        }
